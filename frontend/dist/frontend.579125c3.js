...
window.titlesRecieved = function() {
    axios.get('http://localhost:8082/movies').then((response)=>{
        ...
        titleList.forEach(function(contentTitle) {
            ...
            row.innerHTML = td(contentTitle.primaryTitle) + td(contentTitle.startYear) + td(contentTitle.titleType) + td(contentTitle.genres) + td(contentTitle.runtimeMinutes) + actionButtons(contentTitle.id);
...
window.deleteTitle = function(primaryTitle) {
    if (confirm("Are you sure you want to delete this title?")) axios.delete('http://localhost:8082/movies/' + primaryTitle).then(()=>{
...
window.saveTitle = function(id) {
    ...
    const updated = {
        ...
        startYear: document.getElementById(`edit-year-${id}`).value,
    ...
    axios.put('http://localhost:8082/movies/' + id, updated).then(()=>{
        window.titlesRecieved(); // Refresca la tabla
    }).catch((err)=>alert("Error al guardar: " + err));
};
window.titlesRecieved();

//# sourceMappingURL=frontend.579125c3.js.map
