...
window.deleteTitle = function(primaryTitle) {
    if (confirm("Are you sure you want to delete this title?")) {
        axios.delete('http://localhost:8082/movies/' + primaryTitle)
            .then(() => {
...
window.editTitle = function(id) {
    ...
    row.innerHTML =
        ...
        `<td><input type="number" value="${current.startYear}" id="edit-year-${id}" class="form-control form-control-sm"></td>` +
...
window.saveTitle = function(id) {
    ...
    axios.put('http://localhost:8082/movies/' + id, updated)
        .then(() => {
